...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        default=(
            "configs/detection/slowonly/slowonly_kinetics400-pretrained-"
    ...
    parser.add_argument(
        "--label-map", default="tools/data/ava/label_map.txt", help="label map file"
    )
    parser.add_argument(
...
def main():
    ...
    sampler = [x for x in val_pipeline if get_str_type(x["type"]) == "SampleAVAFrames"][
        0
    ]
